...
class intNMF():
    ...
    def __init__(self, adata, n_topics, reg = None,epochs = 20, init = 'random',l1_weight=1e-4, tol=1e-4,dense=False,random_state = 123, mod1_skew = 1, max_time=None):
        """
        Parameters
        ----------
    ...
    def fit(self, adata, fixed_H_expr=None, fixed_H_im=None, fixed_W = None):
        ...
        for i in range(self.epochs):
            ...
            if fixed_W is None:
                self.theta, theta_it = self._HALS_W(self.theta, rnaHHt, rnaMHt, imgHHt, imgMHt)
            else:
                self.theta = fixed_W
                theta_it = 0
            ...
            if fixed_H_expr is not None:
                ...
                self.phi_expr, self.phi_expr_it = self._HALS(self.phi_expr, B_expr, A_expr)
            ...
            if fixed_H_im is not None:
                ...
                self.phi_im, self.phi_im_it = self._HALS(self.phi_im, B_im, A_im)
    ...
    def _HALS(self, V, UtU, UtM, alpha=0.5, delta=0.1):
    ...
    def _HALS_W(self, W, rnaHHt, rnaMHt, imgHHt, imgMHt,alpha=0.5, delta=0.1):
            """Optimizing min_{V >= 0} ||M-UV||_F^2 with an exact block-coordinate descent schemeUpdate V.
            UtU and UtM are exprensive to compute so multiple updates are calcuated. 
            Parameters:
            -----------
    ...
    def _initialize_nmf(self, X, IM_mat, n_components, fixed_W=None,fixed_H_expr=None,fixed_H_im=None, eps=1e-6,init='random'):
            ...
            if init == 'random':
                ...
                if fixed_W is not None:
                    ...
                    # W = np.maximum(avg * rng.standard_normal((n_samples, n_components)), 0).astype(X.dtype, copy=False)
                if fixed_H_expr is not None:
                    H = fixed_H_expr
                else:
                    H = avg * rng.standard_normal(size=(n_components, X.shape[1])).astype(X.dtype, copy=False)
                    # H = np.maximum(avg * rng.standard_normal((n_components, n_features)), 0).astype(X.dtype, copy=False)
                if fixed_H_im is not None:
                    H_im = fixed_H_im
                else:
                    # H_im = np.maximum(avg_im * rng.standard_normal((n_components, IM_mat.shape[1])), 0).astype(IM_mat.dtype, copy=False)
            ...
            if init == "nndsvd":
                ...
                # U, S, Vt = svds(X, k=n_components, which='LM',random_state=self.random_state)
                # U, Vt = U[:, ::-1], Vt[::-1, :]
                # S = S[::-1]
                U, S,Vt = randomized_svd(IM_mat, n_components,random_state=self.random_state)
        
                # W, H = np.zeros(U.shape), np.zeros(V.shape)


        
                W = np.zeros(U.shape) if fixed_W is None else fixed_W
                H_im = np.zeros(Vt.shape) if fixed_H_im is None else fixed_H_im

                H_expr = np.zeros((n_components, X.shape[1]))

            if fixed_W is None:
                W[:, 0] = np.sqrt(S[0]) * np.abs(U[:, 0])

            if fixed_H_im is None:
                H_im[0, :] = np.sqrt(S[0]) * np.abs(Vt[0, :])
            ...
            # so it can be used as is for initialization.
                ...
                H_expr= safe_sparse_dot(np.linalg.pinv(W), X)
                H_expr[H_expr < eps] = 0
                H = H_expr
